...
$log = array();
$consulta_arq_exc = $zbxCofre->listArquivosExcluidos($logArquivos->consUltDataDel()); 
$consulta_arq_add = $zbxCofre->listArquivosAdicionados($logArquivos->consUltDataAdd());           
    ...
    foreach ($consulta_arq_add as $table) {
        $arq = $table['value'];
        $data_hora_log = $table['data_hora'];
        $linhas = explode("\n", $arq);                
        foreach ($linhas as $key => $value){                    
