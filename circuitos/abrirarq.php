...
$qtd_linha_quebra = 1000;
if(!isset($_SESSION ['pendente'])) { $_SESSION ['pendente'] = ''; }
if(!isset($_SESSION ['arquivo'])) { $_SESSION ['arquivo'] = '';  }
if(!isset($_SESSION ['linha_inicio'])) { $_SESSION ['linha_inicio'] = 0; }
//echo 'Qtd linhas em '.$_SESSION ['arquivo'].': '.count($arquivos).'<br/>'.'<a type="button" class="btn btn-danger" href="confirmaimport.php">voltar <span class="glyphicon glyphicon-backward"></span></a>' ;
if(count($arquivos) > 0){    
foreach($arquivos as $key2 => $img){
    if ($img == $_SESSION ['arquivo']){
        
        $linhas = explode("\n", file_get_contents($img));
        $linha_fim = $_SESSION ['linha_inicio'] + $qtd_linha_quebra;
        if (count($linhas) > $linha_fim){
            $_SESSION ['pendente'] = 'S';                        
        } else {
            $_SESSION ['pendente'] = 'N';
        }
        
        //echo 'Pendente: '.$_SESSION ['pendente'].' QTD. Linhas: '.count($linhas).'| Linha Fim:'.$linha_fim.' Linha inicio: '.$_SESSION ['linha_inicio'];
        foreach ($linhas as $key => $value){
            if(($key >= $_SESSION ['linha_inicio'])&&($key <= $linha_fim )){
               // echo 'linha: '.$key.'<br/>';    
    ...
    if($_SESSION ['pendente'] == 'N'){
        unlink($img);
        unset($_SESSION ['pendente']); 
        unset($_SESSION ['arquivo']); 
        unset($_SESSION ['linha_inicio']);
    }elseif ($_SESSION ['pendente'] == 'S') {
        $_SESSION ['linha_inicio'] = $linha_fim + 1;    
    }
